...
/*
	...
	for( i = 0; i < num; i++ ) {
		...
		if ( read ) {
			...
			nbytes = msg->len;
			dev_dbg( & adap->dev, "pinctrl_fake_spi_at25_xfer(): (write) nbytes %d\n", nbytes );
			offset = ichip->eeprom.mem_address;

			if ( msg == & msg[ 0 ] ) {
				dev_dbg( & adap->dev, "pinctrl_fake_spi_at25_xfer(): (write) msg == & msg[ 0 ]\n" );
				if ( msg->len < 2 ) {
				...
				nbytes -= 2;
				dev_dbg( & adap->dev, "pinctrl_fake_spi_at25_xfer(): (write) offset %04x\n", offset );
				dev_dbg( & adap->dev, "pinctrl_fake_spi_at25_xfer(): (write) nbytes %d\n", nbytes );
			}
...
out:
	dev_dbg( & adap->dev, "pinctrl_fake_spi_at25_xfer(): returning %d\n", r );
	return r;
*/
	return -ENOSYS;
}
...
/*
	...
	ichip = container_of( eeprom, struct pinctrl_fake_spi_chip, eeprom );

	dev_info( & ichip->adapter.dev, "Fake AT24 EEPROM, Copyright (C) 2016, Christopher Friedt, initializing\n" );

	if ( size < SPI_AT24_MEM_SIZE_MIN || size > SPI_AT24_MEM_SIZE_MAX ) {
