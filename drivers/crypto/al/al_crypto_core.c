 ...
 *
 * RMN description:
 * When the Interrupt controller in UDMA is configured in following mode:
 * Group A: Generate MSI-X for the summary of Group D
 * Group B: Generate MSI-X for each bit in the group (Rx Queue interrupt)
 * Group C: Generate MSI-X for each bit in the group (Tx Queue interrupt)
 * Group D: Sent summary to GroupA
 * The Interrupt Disable bit in the PCIe control register is set.
 * As a result the interrupt summary of groupD is masked and bit2 that
 * indicate the status of groupD summary is never set.
 *
 * Software flow:
 * Clear the Interrupt Disable bit in the PCIe control register
 * The MSI-X interrupt enable bit still mask the Legacy interrupts from the
 * UDMA towards the GIC, so the corresponding bits in the GIC can still be used
 * for MSI-X
 *
 * ***************************************************************************/
#ifdef CONFIG_CRYPTO_DEV_AL_RMN_2871
static inline void al_crypto_enable_legacy_interrupts(struct pci_dev *pdev)
{
	u16 pci_command;
...
#ifdef CONFIG_CRYPTO_DEV_AL_RMN_2871
	al_crypto_enable_legacy_interrupts(pdev);
#endif
