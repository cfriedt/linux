...
static int transport_processing_thread(void *param)
{
	...
	if(!(dev->se_sub_dev->se_dev_thread_cpumask == 0) && 
		!(dev->se_sub_dev->se_dev_thread_cpumask == 0xff)){

		if (!zalloc_cpumask_var(&cpumask, GFP_KERNEL)) {
			pr_err("Unable to allocate conn->conn_cpumask\n");
		}else{
			for_each_online_cpu(cpu) {
				if((1 <<cpu) & dev->se_sub_dev->se_dev_thread_cpumask){
			...
			pr_debug("%s: set cpumask %u\n",__func__,dev->se_sub_dev->se_dev_thread_cpumask);
		}
	}
#endif
