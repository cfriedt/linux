		...
		bool ctx_change, bool must_succeed)
{
	...
	if (timeleft == 0) {
		xhci_warn(xhci, "Timeout while waiting for %s command\n",
				ctx_change == 0 ?
					"configure endpoint" :
					"evaluate context");
...
int etxhci_discover_or_reset_device(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	timeleft = wait_for_completion_timeout(
			...
			USB_CTRL_SET_TIMEOUT);
	if (timeleft == 0) {
		xhci_warn(xhci, "Timeout while waiting for reset device command\n");
		spin_lock_irqsave(&xhci->lock, flags);
		/* The timeout might have raced with the event ring handler, so
		 * only delete from the list if the item isn't poisoned.
		 */
...
void etxhci_free_dev(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	list_for_each_entry_safe(lb, next_lb,
			&virt_dev->lb_free_list, list) {
		list_del(&lb->list);
		dma_free_coherent(dev, XHCI_LB_SIZE,
				lb->start, lb->dma_start);
		kfree(lb);
	}
