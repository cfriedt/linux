...
static unsigned int count_lb_trbs_needed(struct xhci_hcd *xhci,
		struct urb *urb, gfp_t mem_flags)
{
	int num_sgs, num_trbs, running_total, count, i;
	...
	if (usb_urb_dir_in(urb) ||
			urb->transfer_buffer_length > XHCI_LB_SIZE)
		return 0;
	...
	lb = NULL;
	virt_dev = xhci->devs[urb->dev->slot_id];
	spin_lock(&virt_dev->lb_lock);
	if (!list_empty(&virt_dev->lb_free_list)) {
		lb = list_first_entry(&virt_dev->lb_free_list,
				struct xhci_lb, list);
		list_del_init(&lb->list);
	}
	spin_unlock(&virt_dev->lb_lock);

	if (!lb) {
		lb = kzalloc(sizeof(struct xhci_lb), mem_flags);
		if (!lb)
			return 0;

		lb->start = dma_alloc_coherent(dev, XHCI_LB_SIZE,
				&lb->dma_start, mem_flags | __GFP_NOWARN);
		if (!lb->start) {
			kfree(lb);
			return 0;
		}
	...
	sg_copy_to_buffer(urb->sg, num_sgs, lb->start,
			XHCI_LB_SIZE);
	lb->dev = virt_dev;

	num_trbs = 0;
	running_total = TRB_MAX_BUFF_SIZE -
		(lb->dma_start & (TRB_MAX_BUFF_SIZE - 1));
...
static int queue_bulk_sg_tx(struct xhci_hcd *xhci, gfp_t mem_flags,
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	struct xhci_virt_device *virt_dev = xhci->devs[slot_id];
	struct xhci_ring *ep_ring;
	unsigned int num_trbs;
	struct urb_priv *urb_priv;
	struct xhci_td *td;
	...
	num_trbs = 0;
	if (virt_dev->use_linear_buffer)
		num_trbs = count_lb_trbs_needed(xhci, urb, mem_flags);
	if (!num_trbs)
	...
	if (urb_priv->lb) {
		addr = (u64) urb_priv->lb->dma_start;
