		...
		struct usb_device *udev, gfp_t flags)
{
	...
	spin_lock_init(&dev->lb_lock);
	INIT_LIST_HEAD(&dev->lb_free_list);
...
#define XHCI_QUIRK_MANUAL_HALT_CLEANUP	(1 << 2)
	...
	if (xhci_is_mass_storage_device(xhci, udev->slot_id))
		virt_dev->use_linear_buffer = 1;
...
void etxhci_urb_free_priv(struct xhci_hcd *xhci, struct urb_priv *urb_priv)
{
	if (urb_priv) {
		if (urb_priv->lb) {
			struct xhci_virt_device *dev = urb_priv->lb->dev;
			spin_lock(&dev->lb_lock);
			list_add_tail(&urb_priv->lb->list,
					&dev->lb_free_list);
			spin_unlock(&dev->lb_lock);
		}
		kfree(urb_priv->td[0]);
		kfree(urb_priv);
	}
